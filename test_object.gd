...
func _ready() -> void:
	...
	add_to_group("cluster")  # Treat test objects as clusters for modem discovery
...
func on_packet_received(packet) -> void:
	packets_processed += 1
	print("[TestObject] Processing packet with power: ", processing_power, " | Total: ", packets_processed)
	var nm = _find_network_manager()
	if nm:
		nm.add_reward(processing_power * 10)
