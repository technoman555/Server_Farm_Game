...
# Called by the NetworkManager's send_text(). The client is the SOURCE,
# so this is triggered when a cluster asks for text and the NM routes
# through this client. The client generates the text and the NM handles delivery.
func send_text_to(target_module: Node) -> void:
	var nm = _find_network_manager()
	if not nm:
		print("[Client] No NetworkManager found!")
		return
	var success = nm.send_text(self, target_module)
	if success:
		texts_sent += 1
		is_connected = true
		print("[Client] Sent text to ", target_module.name, " (total sent: ", texts_sent, ")")
	else:
		is_connected = false
		print("[Client] Failed to send text to ", target_module.name, " — no cable path")
