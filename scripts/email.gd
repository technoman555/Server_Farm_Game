...
var target_module: Node = null # The destination module (internet)
...
func _on_reached_destination() -> void:
	...
	if target_module and is_instance_valid(target_module) and target_module.has_method("on_email_received"):
		target_module.on_email_received(self)
	else:
		# No valid target — just clean up
		print("[Email] Reached destination but no valid target module")
		queue_free()