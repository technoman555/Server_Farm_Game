...
func _on_reached_destination() -> void:
	...
	if target_module and is_instance_valid(target_module) and target_module.has_method("on_packet_received"):
		target_module.on_packet_received(self)
	else:
		# No valid target — just clean up
		print("[Packet] Reached destination but no valid target module")
		queue_free()
